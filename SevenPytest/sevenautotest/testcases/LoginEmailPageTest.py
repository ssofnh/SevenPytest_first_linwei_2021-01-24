...
class LoginEmailPageTest(BaseTestCase):
    ...
    @pytest.mark.testcase("成功登陆测试", author="linwei", editor="")
    def test_successfully_login(self, testdata):
        ...
        page = LoginEmailPage()
        page.chrome().maximize_window().open_url(url)
        # page.firefox().maximize_window().open_url(url)
        # page.ie().maximize_window().open_url(url)
        # page.Edg().maximize_window().open_url(url)
        page.screenshot("successfully baidu.png")
        page.sleep(3)
